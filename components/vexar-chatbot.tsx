  ...
  const send = useCallback(async () => {
    ...
    try {
      ...
      if (error.message === 'API key not configured') {
        errorMessage = "🤖 **API Key Required**\n\nTo use VEXAR, you need to add your OpenAI API key:\n\n1. Create `.env.local` file\n2. Add: `NEXT_PUBLIC_OPENAI_API_KEY=your_key_here`\n3. Get key from: https://platform.openai.com/\n\nThen restart the server!"
      } else if (error.message === 'Invalid API key') {
        errorMessage = "🔑 **Invalid API Key**\n\nPlease check your OpenAI API key in the `.env.local` file. Make sure it's correct and has credits available."
